...
public class CategoryController {
    ...
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<ApiResponse<List<TicketCategoryDto>>> getSubcategories(@PathVariable("id") Long id) {
        List<TicketCategoryDto> subcategories = categoryService.getSubcategoriesByCategoryId(id);
    ...
    @PreAuthorize("hasRole('OPERATIONS_SUPERVISOR') or hasRole('CUSTOMER_SUPPORT_MANAGER')")
    public ResponseEntity<ApiResponse<TicketCategoryDto>> createSubcategory(
            @PathVariable("id") Long id,
            @Valid @RequestBody CreateCategoryRequest request) {
        TicketCategoryDto subcategory = categoryService.createSubcategory(id, request);
